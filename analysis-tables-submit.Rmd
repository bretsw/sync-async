...
tweeters_df <- df_oo %>% 
    ...
    group_by(is_sync) %>% 
    summarize(unique_tweeters = (screen_name %>% unique %>% length)) # this can be replaced with n()

sync_tweeters_n <- tweeters_df %>% 
    filter(is_sync==1) %>% 
    pull(unique_tweeters)

async_tweeters_n <- tweeters_df %>%
    filter(is_sync==0) %>% 
    pull(unique_tweeters)
...
sync_only_original <- df_oo %>% # starts with _oo dataframe
    ...
    mutate(screen_name = tolower(screen_name)) %>% # this is maybe a bit easier to reason about than below
    # mutate(screen_name = screen_name %>% tolower) %>%  
    count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1) %>% 
    nrow()
