...
d <- d %>% 
    mutate(mentions_count = sapply(str_split(d$mentions_screen_name, " "), length),
           hashtags_count = sapply(str_split(d$hashtags, " "), length), 
           urls_count = sapply(str_split(d$urls_url, " "), length)
    )
...
m <- manova(cbind(Social, `Cognitive Processing`, `Positive affect`, `Negative affect`, `Work-related Concerns`) ~ is_sync,
            data = prepped_data1)
summary(m)

fit1 = hotelling.test(. ~ is_sync, data = prepped_data1, perm = T)
fit1
