...
sync_n <- df_ss %>% tabyl(is_sync) %>% filter(is_sync==1) %>% pull(n)
async_n <- df_ss %>% tabyl(is_sync) %>% filter(is_sync==0) %>% pull(n)
sync_n; async_n
...
df_ss %>% count(is_sync, screen_name)
