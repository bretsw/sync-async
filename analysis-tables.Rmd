...
author: "Joshua M. Rosenberg, K. Bret Staudt Willet, & Spencer P. Greenhalgh"
date: "4/24/2019"
...
Only need to run once, then the file should be downloaded to your working directory. 

```{r, eval = FALSE}
# must have generated an API token from an OSF account that has been added to the repository here: https://developer.osf.io/#tag/General-Usage
# Should see this message:
# "Automatically registered OSF personal access token"

# First time setup: (http://centerforopenscience.github.io/osfr/articles/auth.html)
#usethis::edit_r_environ(scope='project')  # to add token to .renviron file
#  Add 1 line, replacing <token> with your token: 
#OSF_PAT="<token>"

osf_retrieve_file("https://osf.io/ktm2s/?pid=m25zy") %>% 
    osf_download(path = "sync-async-dataset.csv")
```


```{r, include=FALSE}
df_ss <- read_csv("sync-async-dataset.csv", skip=1)
...
proc_tweets <- function(d) {
  d %>%
    mutate(mentions_count = map_int(mentions_screen_name, length),
           ...
           is_orig = if_else(!is_retweet, TRUE, FALSE))
}

df_ss <- proc_tweets(df_ss)

df_ss %>% count(is_retweet, is_quote, is_reply, is_orig, type)
```
...
sync_n <- df_ss %>% janitor::tabyl(is_sync) %>% filter(is_sync==1) %>% pull(n)
async_n <- df_ss %>% janitor::tabyl(is_sync) %>% filter(is_sync==0) %>% pull(n)
sync_n; async_n
...
tweeters_df <- df_oo %>% 
=======
df_ss %>% pull(screen_name) %>% tolower() %>% unique() %>% length()

tweeters_df <- df_ss %>% 
>>>>>>> a35522749096cc6ee01b530008ebcd523de00edc
...
both <- df_ss %>% count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 2)
both_n <- both %>% nrow()

only_one <- df_ss %>% count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1)

sync_only_n <- df_ss %>% semi_join(only_one, by="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==1) %>%
...
async_only_n <- df_ss %>% semi_join(only_one, by="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==0) %>%
    count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1) %>% 
...
only_one_original <- df_ss %>% count(screen_name, is_retweet) %>% 
    count(screen_name) %>% filter(n == 1)

both_original <- df_ss %>% count(screen_name, is_retweet) %>% 
    count(screen_name) %>% filter(n == 2)

both_original_n <- both_original %>% nrow()

sync_only_original <- df_ss %>%
...
sync_only_nonoriginal <- df_ss %>% semi_join(only_one, by="screen_name") %>%
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==1 & is_retweet) %>%
    count(screen_name, is_sync) %>% 
...
sync_only_both <- df_ss %>% semi_join(both_original, by="screen_name") %>%
    semi_join(only_one, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==1) %>%
    count(screen_name, is_sync) %>% 
...
async_only_nonoriginal <- df_ss %>% semi_join(only_one, by="screen_name") %>%
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==0 & is_retweet) %>%
    count(screen_name, is_sync) %>% 
...
async_only_both <- df_ss %>% semi_join(both_original, by="screen_name") %>%
    semi_join(only_one, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==0) %>%
    count(screen_name, is_sync) %>% 
...
both_nonoriginal <- df_ss %>% semi_join(both, by="screen_name") %>%
    semi_join(both, by="screen_name") %>% 
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_retweet) %>%
...
both_both <- df_ss %>% semi_join(both_original, by="screen_name") %>%
    semi_join(both, by="screen_name") %>% 
    mutate(screen_name = screen_name %>% tolower) %>%
    count(screen_name, is_retweet) %>% 
    count(screen_name) %>% 
...
**Mean number of original tweets per tweeter**

```{r}
df_ss %>% filter() %>% count(is_sync)
...
```{r}
df_ss$created_at_m <- df_ss$created_at - lubridate::hours(5)
df_ss$date_r <- df_ss$created_at_m %>% floor_date("day")
...
df_ss <- df_ss %>% 
    mutate(mentions_count = sapply(str_split(df_ss$mentions_screen_name, " "), length),
           hashtags_count = sapply(str_split(df_ss$hashtags, " "), length), 
           urls_count = sapply(str_split(df_ss$urls_url, " "), length)
    )

is.na(df_ss$is_sync) %>% which()  # check to make sure there are no NAs
...
# RQ2: Content; RQ3: Interaction; RQ4: Portals
...
```{r}
df_ss %>% filter(is_quote==TRUE) %>% nrow()  # total number of quote tweets
sync_quote_n <- df_ss %>% filter(is_quote==TRUE) %>% 
    janitor::tabyl(is_sync) %>% filter(is_sync==1) %>% pull(n)
async_quote_n <- df_ss %>% filter(is_quote==TRUE) %>%
