...
df_oo <- df_ss %>% mutate(type = ifelse(type == "RETWEET", 0, 1))

df_oo %>% pull(screen_name) %>% tolower() %>% unique() %>% length()
...
both <- df_oo %>% count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 2)
both_n <- both %>% nrow()

only_one <- df_oo %>% count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1)

sync_only_n <- df_oo %>% semi_join(only_one, by="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==1) %>%
...
async_only_n <- df_oo %>% semi_join(only_one, by="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==0) %>%
    count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1) %>% 
...
only_one_original <- df_oo %>% count(screen_name, type) %>% 
    count(screen_name) %>% filter(n == 1)

both_original <- df_oo %>% count(screen_name, type) %>% 
    count(screen_name) %>% filter(n == 2)

both_original_n <- both_original %>% nrow()

sync_only_original <- df_oo %>% # starts with _oo dataframe
    semi_join(only_one, by="screen_name") %>% 
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==1 & type== 1) %>%
    count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1) %>% 
    nrow()

sync_only_nonoriginal <- df_oo %>% semi_join(only_one, by="screen_name") %>%
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==1 & type == 0) %>%
    count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1) %>% 
    nrow()

sync_only_both <- df_oo %>% semi_join(both_original, by="screen_name") %>%
    semi_join(only_one, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==1) %>%
    count(screen_name, is_sync) %>% 
...
async_only_original <- df_oo %>% 
    semi_join(only_one, by="screen_name") %>% 
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==0 & type== 1) %>%
    count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1) %>% 
    nrow()

async_only_nonoriginal <- df_oo %>% semi_join(only_one, by="screen_name") %>%
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==0 & type == 0) %>%
    count(screen_name, is_sync) %>% 
    count(screen_name) %>% filter(n == 1) %>% 
    nrow()

async_only_both <- df_oo %>% semi_join(both_original, by="screen_name") %>%
    semi_join(only_one, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(is_sync==0) %>%
    count(screen_name, is_sync) %>% 
...
both_original_ <- df_oo %>% 
    semi_join(both, by="screen_name") %>% 
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(type== 1) %>%
    count(screen_name, type) %>% 
    count(screen_name) %>% 
    nrow()

both_nonoriginal <- df_oo %>% semi_join(both, by="screen_name") %>%
    semi_join(both, by="screen_name") %>% 
    semi_join(only_one_original, by ="screen_name") %>%
    mutate(screen_name = screen_name %>% tolower) %>%
    filter(type== 0) %>%
...
both_both <- df_oo %>% semi_join(both_original, by="screen_name") %>%
    semi_join(both, by="screen_name") %>% 
    mutate(screen_name = screen_name %>% tolower) %>%
    count(screen_name, type) %>% 
    count(screen_name) %>% 
...
```{r}
df_ss %>% filter(type == "ORIG") %>% count(is_sync)
