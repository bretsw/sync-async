...
m6 <- glmer(Likes ~ is_sync + (1|screen_name), family = "poisson", data = prepped_data)
...
get_kr_df <- function(model_object) {
  ...
  L <- as.data.frame(L)
  out <- purrr::map_dbl(pbkrtest::get_Lb_ddf, object = model_object)
...
get_stats <- function(m) {
  x <- broom::tidy(m)
  o <- filter(x, group == "fixed", term != "(Intercept)")
  o <- mutate(o, est_se = str_c(round(estimate, 3), " (", round(std.error, 3), ")")) 
  o <- as.vector(unlist(select(o, est_se)))
  i <- round(as.vector(performance::icc(m))[[2]][1], 3)
  
  
  kr_df <- get_kr_df(m)
  d <- data.frame(est_se = o, icc = i, df = kr_df)
  d <- mutate(p_val = t_dist_p(statistic, df))
  return(d)
}

l <- list(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11) %>% 
  map_df(get_stats)
